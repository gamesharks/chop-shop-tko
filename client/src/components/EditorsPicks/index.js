...
const ParagraphSection = ({ name, image, text }) => {
  return (
    <div style={{ display: 'flex', alignItems: 'center', marginBottom: '2rem' }}>
      <img src={image} alt={`${name}'s profile`} style={{ marginRight: '1rem', borderRadius: '50%', width: '50px', height: '50px' }} />
      <div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
      <div style={{ marginBottom: '1rem', maxWidth: '50vw', color: '#E53935' }}>
        <ParagraphSection
          name="Gerardo"
          image="https://via.placeholder.com/50" // replace w pic link here if we keep this format
